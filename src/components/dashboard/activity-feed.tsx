  ...
  return (
    <section className="rounded-xl border border-slate-800 bg-slate-900">
      <header className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
        <h2 className="text-sm font-semibold text-white">Actividade recente</h2>
        <Link
          href="/inbox"
      ...
      {loading || !items ? (
        ...
        <div className="p-5">
          <EmptyState
            icon={Inbox}
            title="Ainda não existe actividade"
            hint="A actividade de mensagens, negócios, campanhas e automações aparecerá aqui."
          />
        </div>
      ) : (
        <>
          ...
          <footer className="flex items-center justify-between border-t border-slate-800 px-5 py-3 text-xs">
            <span className="text-slate-500 tabular-nums">
              A mostrar {visible.length} de {totalLoaded}
              {totalLoaded === 50 ? '+' : ''}
            </span>
